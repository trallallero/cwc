...
class CTkButton(CTkBaseClass):
    ...
    def __init__(self,
                 ...
                 text_color_disabled: Optional[Union[str, Tuple[str, str]]] = 'grey',
        ...
        if isinstance(self._image, CTkImage):
            ...
            self._image.configure(state=state)
    ...
    def _update_image(self, state=None):
        if state and self._image:
            self._image.configure(state=state)
        ...
        if self._cursor_manipulation_enabled:
            if self._state == tkinter.DISABLED:
                if sys.platform == "darwin":
                    self.configure(cursor=self.__get_forbid_curs())
    ...
    def _on_enter(self, event=None):
        ...
        elif self._hover is True and self._state == "disabled":
            if self.cget("cursor") != self.__get_forbid_curs():
                self.configure(cursor=self.__get_forbid_curs())
    ...
    def __get_forbid_curs(self):
        if globals()['__FORBID_CURS'] == '':
            try:
                tkinter.Label(cursor='@images/forbid.cur')
