...
class CTkImage:
    ...
    def __init__(self,
                 ...
                 light_image_disabled: "Image.Image" = None,
                 dark_image_disabled: "Image.Image" = None,
        ...
        self._light_image_disabled = light_image_disabled
        self._dark_image_disabled  = dark_image_disabled
        ...
        self._scaled_light_photo_images_disabled: Dict[Tuple[int, int], ImageTk.PhotoImage] = {}
        self._scaled_dark_photo_images_disabled : Dict[Tuple[int, int], ImageTk.PhotoImage] = {}
    ...
    def add_configure_callback(self, callback: Callable):
        ...
        if callback not in self._configure_callback_list:
    ...
    def _check_images(self):
        ...
        if self._light_image_disabled is not None and not isinstance(self._light_image_disabled, Image.Image):
            raise ValueError(f"CTkImage: light_image_disabled must be instance if PIL.Image.Image, not {type(self._light_image_disabled)}")
        if self._dark_image_disabled is not None and not isinstance(self._dark_image_disabled, Image.Image):
            raise ValueError(f"CTkImage: dark_image_disabled must be instance if PIL.Image.Image, not {type(self._dark_image_disabled)}")
    ...
    def _get_scaled_light_photo_image_disabled(self, scaled_size: Tuple[int, int]) -> "ImageTk.PhotoImage":
        if scaled_size in self._scaled_light_photo_images_disabled:
            return self._scaled_light_photo_images_disabled[scaled_size]
        else:
            self._scaled_light_photo_images_disabled[scaled_size] = ImageTk.PhotoImage(self._light_image_disabled.resize(scaled_size))
            return self._scaled_light_photo_images_disabled[scaled_size]
    ...
    def _get_scaled_dark_photo_image_disabled(self, scaled_size: Tuple[int, int]) -> "ImageTk.PhotoImage":
        if scaled_size in self._scaled_dark_photo_images_disabled:
            return self._scaled_dark_photo_images_disabled[scaled_size]
        else:
            self._scaled_dark_photo_images_disabled[scaled_size] = ImageTk.PhotoImage(self._dark_image_disabled.resize(scaled_size))
            return self._scaled_dark_photo_images_disabled[scaled_size]
    ...
    def create_scaled_photo_image(self, widget_scaling: float, appearance_mode: str) -> "ImageTk.PhotoImage":
        ...
        if appearance_mode == "light" and self._light_image is not None:
            if self.state == 'normal':
                ...
                return self._get_scaled_light_photo_image_disabled(scaled_size)
        elif appearance_mode == "light" and self._light_image is None:
            if self.state == 'normal':
                return self._get_scaled_dark_photo_image(scaled_size)
            else:
                return self._get_scaled_dark_photo_image_disabled(scaled_size)
