...
class WidgetList:
    ...
    def remove(self, item):
        try:
            indexes = [i for i, w in enumerate(self.widgetlist) if w.widget == item]
            if indexes:
                self.widgetlist.pop(indexes[0])
        except ValueError:
            pass
...
class WidgetFrame:
    ...
    def removeWidget(self, widget):
        self.widgets.remove(item=widget)
    ...
    def addFrame(self, name: str, row: int = None, col: int = None, padx=(20,20), pady=(20,20), sticky="nsew",
                 rowspan: int = 1, colspan: int = 1, widgetkwargs: dict = None, gridkwargs: dict = None, use_tk=False):
        """
        ...
        if use_tk:
            widget = tk.Frame(self.master, **widgetkwargs)
        else:
        ...
        self.widgets.append(Widget(frame, name, row, col, rowspan, colspan, name))
        return frame
    ...
    def Checkbutton(self, text: str, variable: tk.Variable, command = None, args=(), row: int =None, col: int =0,
        ...
        """
        widgetkwargs, gridkwargs = noneDict(widgetkwargs, gridkwargs)
        widget = ttk.Checkbutton(self.master, text=text, variable=variable, command=partial(command, *args), cursor='hand2',
    ...
    def Radiobutton(self, text: str, variable: tk.Variable, value, command=None, args=(), row: int = None,
        ...
        widget.bind('<Enter>', lambda event, widget=widget, variable=variable, value=value:
            widget.configure(cursor='arrow' if variable.get() == value else 'hand2')
        )
        row, col = self.getRow(row, col, rowspan, colspan)
        widget.grid(row=row, column=col, padx=padx, pady=pady, sticky=sticky, rowspan=rowspan, columnspan=colspan, **gridkwargs)
    ...
    def Combobox(self, values: list, variable: tk.Variable, default = 0, row: int = None, col: int = None, padx=10,
        ...
        # Remove the focus when item selected to avoid an ugly visual effect: the combo stays highlighted when hovering over another combo
        widget.bind('<<ComboboxSelected>>', lambda event: self.master.focus())
        widget.bind('<Leave>'             , lambda event: self.master.focus())
        widget.bind('<Enter>'             , lambda event, widget=widget:
            widget.configure(cursor=self.__get_forbid_curs() if str(widget.cget('state')) == 'disabled' else 'hand2')
        )
